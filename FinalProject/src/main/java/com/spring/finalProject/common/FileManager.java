...
public class FileManager {
	...
	// 리턴값 : WAS에 저장된 새로운 파일명(ex: 2021110809271535243254235235234.png)
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
		...
		newFilename += fileExt; 
		// newFilename 은 예를 들면 2021110809271535243254235235234.png 와 같이 된다.
		...
		// pathname 은 예를 들면, 
		// C:\NCS\workspace(spring)\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\Board\resources\files\2021110809271535243254235235234.png
	...
	public boolean doFileDownload(String saveFilename, String originalFilename, String path, HttpServletResponse response) {
		...
		} catch (UnsupportedEncodingException e) { // getBytes("UTF-8"),"8859_1" 이부분을 잘못 썼을 경우
		...
		try {
			...
			if(file.exists()) { // 실제로 다운로드 할 해당 파일이 존재한다면
				...
				// ServletOutputStream 은 다운로드 되어질 파일을 클라이언트로 보내어주는 출력 스트림용
				...
				souts.flush(); // ServletOutputStream souts 에 기록(저장)해둔 내용을 클라이언트로 내보낸다.
	...
	public String doFileUpload(InputStream is, String originalFilename, String path) throws Exception {
	    ...
	    fos.flush(); // autoflush 이기 때문에 안써도 괜찮다.
